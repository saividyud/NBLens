if __name__ == '__main__':
    ...
    SS1_str = ['0.2', '0.6', '1.0', '1/0.6', '1/0.2']
    SS2_str = ['0.2', '0.6', '1.0', '1/0.6', '1/0.2']
    ALPHAS = [0.0, 45.0, 90.0, 135.0, 180.0]
    TOTAL_SIMS = len(SS1_str) * len(SS2_str) * len(ALPHAS)  # 125

    ss1 = [numexpr.evaluate(s) for s in SS1_str]
    ...
    headers = ['s1', 's2', 'alpha', 'source_radius', 'LD_coeff'] + [f'{thresh:.1e}' for thresh in thresholds]
    output = []

    #%% Looping through parameter space
    with open(f'./Unity Analysis/Part 5/Data Files/analysis_output_{multiplier:.0e}.csv', 'w', newline='') as csvfile:
        ...
        for i, s1 in enumerate(ss1):
            for j, s2 in enumerate(ss2):
                for k, alpha in enumerate(alphas):
                    print('=' * 50)
                    print(f'Processing s1={s1}, s2={s2}, alpha={alpha}...')
                    ...
                    #%% Reading in binary lens simulation
                    ...
                    output.append([SS1_str[i], SS2_str[j], ALPHAS[k], radius, LD_coeff] + fractional_area_above_thresholds.tolist())
